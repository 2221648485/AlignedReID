...
## Datasets
parser.add_argument('--root', type=str, default='resource',
...
# Architecture
parser.add_argument('-a', '--arch', type=str, default='resnet152', choices=models.get_names())
# Miscs
...
def main():
    ...
    if use_gpu:
        print(f"Current using GPU {args.gpu_devices}")
    ...
    # 读取对应数据集
    dataset = data_manager.init_img_dataset(name=args.dataset, root=args.root)
    # 图片增强处理
    transforms_train = v2.Compose([
        v2.RandomRotation(45),  # 随机进行-45到45的旋转
        v2.RandomResizedCrop((args.height, args.width)),  # 随机裁剪
        v2.RandomHorizontalFlip(p=0.5),  # 随机进行水平翻转
        v2.RandomVerticalFlip(p=0.5),  # 随机进行垂直翻转
        v2.ColorJitter(brightness=0.2, contrast=0.1, saturation=0.1, hue=0.1),  # 进行亮度,对比度,饱和度,色相的调整
    ...
    # 降低学习率
    if args.stepsize > 0:
        scheduler = optim.lr_scheduler.StepLR(optimizer, args.stepsize, args.gamma)
    ...
    print(f"Finished. Total elapsed time (h:m:s): {elapsed}. Training time (h:m:s): {train_time}.")
